    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-3.5 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto space-y-2.5 sm:space-y-3 mb-6 sm:mb-10 reveal">
          ...
          <p className="text-[#B8B0A4] text-xs sm:text-base lg:text-lg">
        ...
        {/* Filter Bar */}
        <div className="bg-[#111114] p-2.5 sm:p-4 rounded-2xl border border-[#2A2A2E] flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 mb-5 sm:mb-8 reveal reveal-stagger-1">
          <div className="flex gap-1.5 sm:gap-2 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0 scrollbar-none">
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => setSelectedCat(cat.id)}
                className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl text-[11px] sm:text-xs font-semibold whitespace-nowrap shrink-0 transition-all duration-200 cursor-pointer ${
                  selectedCat === cat.id
          ...
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#1A1A1E] hover:bg-[#2A2A2E] text-[#C8A45C] text-[11px] sm:text-xs font-bold border border-[#2A2A2E] cursor-pointer transition-all hover-lift"
          >
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full p-3.5 sm:p-6 text-left flex items-start sm:items-center justify-between gap-3 sm:gap-4 cursor-pointer group"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-6">
                    <span className="font-mono text-[11px] sm:text-xs font-bold text-[#C8A45C] uppercase shrink-0 transition-transform duration-200 group-hover:scale-105">
                      {law.numberStr}
                    </span>
                    <div>
                      <h4 className="font-editorial-serif font-bold text-sm sm:text-lg text-[#F0EBE0] group-hover:text-[#C8A45C] transition-colors">
                        {law.titleEn}
                      </h4>
                      <p className="text-[11px] sm:text-sm font-bengali-serif font-semibold text-[#C8A45C]/70">
                        {law.titleBn}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 shrink-0 mt-0.5 sm:mt-0">
                    ...
                    <div
                      className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-[#08080A] border border-[#2A2A2E] flex items-center justify-center text-[#8A8278] transition-all duration-300 ${
                ...
                >
                  <div className="overflow-hidden">
                    <div className="px-3.5 sm:px-6 pb-4 sm:pb-6 pt-1 text-xs sm:text-sm leading-relaxed text-[#B8B0A4] space-y-2.5 sm:space-y-3 border-t border-[#2A2A2E]/60 bg-[#111114]">
                      <div className="p-3 sm:p-4 rounded-xl bg-[#08080A] border border-[#2A2A2E] text-[#C4BCB0] font-medium leading-[1.75]">
                        {law.detailedDescBn || law.shortDescBn}
                      </div>
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pt-1">
                        <span className="text-[11px] sm:text-xs text-[#8A8278]">
                          সম্পূর্ণ ৫০৯ পৃষ্ঠার ডিজিটাল বইতে রয়েছে প্রতিটি সূত্রের বিস্তারিত কেস স্টাডি।
                        </span>
                        <button
                          onClick={onOpenOrderModal}
                          className="text-[11px] sm:text-xs font-bold text-[#C8A45C] hover:text-[#D4AF6E] hover:underline inline-flex items-center gap-1 cursor-pointer transition-colors group shrink-0"
                        >
        ...
        {/* CTA */}
        <div className="mt-6 sm:mt-8 text-center reveal reveal-stagger-3">
          <button
            onClick={onOpenAllLawsModal}
            className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-full btn-gold text-xs sm:text-sm cursor-pointer hover-lift btn-shimmer group shadow-lg"
          >
