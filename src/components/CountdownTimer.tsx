...
function formatDigit(num: number, lang: "en" | "bn" = "bn"): string {
...
export default function CountdownTimer({
  ...
  const defaultLabel = lang === "en" ? "Ends in" : "অফার শেষ হতে বাকি";
  ...
  if (variant === "inline") {
    return (
      <span className="inline-flex items-center gap-1 sm:gap-1.5 text-[#C8A45C] font-mono text-[11px] sm:text-xs font-bold">
        ...
        <span>
          {formatDigit(timeLeft.days, lang)}d : {formatDigit(timeLeft.hours, lang)}h : {formatDigit(timeLeft.minutes, lang)}m : {formatDigit(timeLeft.seconds, lang)}s
        </span>
      </span>
    );
  }

  // ─── Compact Pill Variant (For Checkout Header) ───────────────────────────
  if (variant === "compact-pill") {
    return (
      <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-[#141418] border border-[#C8A45C]/30 text-xs text-[#F0EBE0] shadow-sm max-w-full">
        ...
        {displayLabel && (
          <span className="text-[#A8A095] text-[10px] sm:text-[11px] font-sans font-medium shrink-0">
            {displayLabel}:
          </span>
        )}
        <span className="font-mono font-bold text-[#C8A45C] tracking-wider text-[11px] sm:text-xs whitespace-nowrap">
          {formatDigit(timeLeft.days, lang)}d {formatDigit(timeLeft.hours, lang)}h {formatDigit(timeLeft.minutes, lang)}m {formatDigit(timeLeft.seconds, lang)}s
        </span>
      </div>
    );
    ...
    <div className="space-y-1 sm:space-y-1.5">
      {displayLabel && (
        <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-[#A8A095] font-semibold">
          ...
          <span className="text-[9px] sm:text-[10px] text-[#C8A45C] uppercase tracking-wider font-bold">
            {lang === "en" ? "Limited Offer" : "স্বয়ংক্রিয় অফার"}
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-1 sm:gap-2 text-center">
        {[
          ...
          { label: lang === "en" ? "Hours" : "ঘণ্টা", value: timeLeft.hours },
          { label: lang === "en" ? "Mins" : "মিনিট", value: timeLeft.minutes },
          { label: lang === "en" ? "Secs" : "সেকেন্ড", value: timeLeft.seconds },
        ].map((unit, idx) => (
          <div
            key={idx}
