...
export default function CountdownTimer({
  ...
  if (variant === "inline") {
    return (
      <span className="inline-flex items-center gap-1.5 text-[#C8A45C] font-mono text-xs font-bold">
        <Clock className="w-3.5 h-3.5 animate-pulse text-[#C8A45C]" />
        <span>
    ...
    <div className="space-y-1.5">
      {label && (
        <div className="flex items-center justify-between text-[10px] sm:text-[11px] text-[#A8A095] font-semibold">
          ...
          <span className="text-[10px] text-[#C8A45C] uppercase tracking-wider font-bold">
            স্বয়ংক্রিয় লাইভ অফার
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-1.5 sm:gap-2 text-center">
        {[
          ...
          <div
            key={idx}
            className="p-1.5 sm:p-2 rounded-xl bg-[#141418] border border-[#2A2A2E] flex flex-col items-center justify-center group hover:border-[#C8A45C]/40 transition-colors"
          >
            <span className="font-mono font-black text-sm sm:text-base text-[#F0EBE0] group-hover:text-[#C8A45C] transition-colors leading-none">
              {toBengaliDigit(unit.value)}
            </span>
            <span className="text-[9px] sm:text-[10px] text-[#8A8278] font-medium mt-1 leading-none">
              {unit.label}
            </span>
          </div>
        ))}
